...
export default function LacakPaket() {
  ...
  const handleLacak = (e: React.FormEvent) => {
    ...
    if (resiInput.trim().toUpperCase() === 'CKL2026040001') {
      ...
      setShowResult(false);
      // Optional: you could add an error toast here if needed
    }
  };
        ...
        {showResult && (
          <div className="flex flex-col lg:flex-row gap-6 md:gap-8 items-start animate-in fade-in slide-in-from-bottom-8 duration-500">
            ...
            <div className="w-full lg:flex-[2] flex flex-col gap-6 md:gap-8">
              ...
              {/* Card Bantuan */}
              <div className="bg-[#e4ece8] rounded-[24px] md:rounded-[32px] p-6 md:p-8 flex justify-between items-center shadow-inner">
                <div>
                  <h4 className="font-bold text-gray-900 text-[15px] md:text-[17px]">Butuh bantuan?</h4>
                  <p className="text-[13px] text-gray-600 mt-1">Hubungi CS 24/7 kami</p>
                </div>
                <button 
                  onClick={() => setIsChatOpen(true)}
                  className="bg-white text-[#1db372] font-bold text-[13px] md:text-[14px] px-6 py-3 rounded-full hover:bg-gray-50 transition-colors shadow-sm"
                >
                  Live Chat
                </button>
              </div>
